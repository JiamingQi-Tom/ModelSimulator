...
def constraints(param):
    ...
    # Positions
    p = CalcPosition(s1, param, symbolic=True)
...
def plotDLO(param):
    ...
    for i in range(N + 1):
        s = ds * i
        P = CalcPosition(s, param, symbolic=False)
...
def CalcPosition(s, param, symbolic=False):
    ...
    for i in range(int(np.round(float(Decimal(str(s)) / Decimal(str(ds))))) + 1):
        ...
        if symbolic:
            zeta[0, 0] = ca.sin(theta_l) * ca.cos(phi_l)
            zeta[1, 0] = ca.sin(theta_l) * ca.sin(phi_l)
            zeta[2, 0] = ca.cos(theta_l)
            p = p + (1 + epsilon_l) * zeta[:, 0] * ds

...
def dlodynamics_2D(x1, y1, x2, y2, langle, rangle, cable_length, init=np.ones((1, 2 * 4 + 2)) / 10, format=1):
    ...
    return centerline, contour, para_a
...
if __name__ == '__main__':
    ...
    centerline, contour, para1 = dlodynamics_2D(0.1, 0.0, 0.7, 0.0, np.pi/6, -np.pi/6, 1.5, para0, format=2)
    plt.xlim([-0.1, 0.8])
    plt.ylim([-0.1, 0.8])
    plt.plot(centerline[:, 0], centerline[:, 1], 'g')
    plt.fill(contour[:, 0], contour[:, 1], 'r')
    plt.scatter(contour[:, 0], contour[:, 1], c='k')
    plt.axis('square')
    plt.grid()
    print(time() - tic)
    plt.pause(0)

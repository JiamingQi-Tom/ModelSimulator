...
%   a) flexural energy
%   b) torsion energy
%   c) extensional energy - supposed to be gravitational energy
%
% param: vecteur of the serie parameters - p=[a_phi, a_theta, a_psi, a_epsilon] is 1x4n vector 
%%
...
for s=s0:ds:s1
    ...
    kappa_2 = dtheta^2 + dphi^2 * sin(theta)^2;
    ...
    if Re ~= 0.0
        epsilon = f_evaluate(s,a_epsilon,0);
    ...
    if D ~= 0.0
        ...
        for sl=s0:ds:s
            if Re ~= 0
                epsilon_l = f_evaluate(sl, a_epsilon, 0);
            end
